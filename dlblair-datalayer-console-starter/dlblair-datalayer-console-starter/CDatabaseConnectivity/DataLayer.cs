...
namespace edu.northeaststate.dlblair.cDatabaseConnectivity
{
    ...
    internal class DataLayer
    {
        ...
        public async Task<List<Rating>>? GetAllRatingsByARatingAsync(int? ratingNum)
        {
            ...
            try
            {
                // checks if param is null
                if (ratingNum == null)
                {
                    throw new ArgumentNullException("rating can not be null");
                }

                // guarenttes release of resources at end of scope
                using MySqlConnection conn = new MySqlConnection(connectionString);

                // db connection open
                conn.Open();

                // creates command object with stored procedure
                using MySqlCommand cmd = new MySqlCommand("spGetAllRatingsByARating", conn);

                // sets command object
                cmd.CommandType = CommandType.StoredProcedure;

                // adds param to command that's passed to stored procedure
                cmd.Parameters.Add(new MySqlParameter("Rating", ratingNum));
        ...
        public async Task<List<Rating>>? GetAllRatingsByArticleIDAsync(int? ArticleID)
        {
            ...
            try
            {
                ...
                // adds param to command that's passed to stored procedure
                cmd.Parameters.Add(new MySqlParameter("ArticleID", ArticleID));
    ...
    /// <exception cref="Exception"></exception>
        public async Task<int>? PostANewRatingAsync(Rating? rating)
            ...
            try
            {
                ...
                if (rating == null)
                {
                    throw new ArgumentNullException("New rating can not be null.");
                }
                ...
                // create a command object identifying the stored procedure
                using MySqlCommand cmd = new MySqlCommand("spPostNewRating", conn);

                // set the command object so it knows to execute a stored procedure
                cmd.CommandType = CommandType.StoredProcedure;

                // add parameters to command, which will be passed to the stored procedure
                cmd.Parameters.Add(new MySqlParameter("ArticleID", rating.ArticleID));
                cmd.Parameters.Add(new MySqlParameter("UserID", rating.UserID));
                cmd.Parameters.Add(new MySqlParameter("Rating", rating.UserRating));
        ...
        public async Task<List<UserLevel>>? GetAllUserLevelsAsync()
        {
            ...
            try
            {
                string connectionString = @"server=localhost;uid=root;pwd=335577;database=neArticleJet";
                //using guarentees the release of resources at the end of scope 
                using MySqlConnection conn = new MySqlConnection(connectionString);
